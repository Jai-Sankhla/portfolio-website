...
export default function Timeline() {
  return (
    <div className="relative">
      <div className="absolute left-[7px] top-3 bottom-3 w-px bg-[#D4D0CA] dark:bg-[#3A3836]" />
        ...
        {experience.map((item, i) => (
          <ScrollReveal key={i} delay={i * 0.1}>
            <div className="relative pl-8">
              <div
                className="absolute left-0 top-1.5 w-[15px] h-[15px] rounded-full border-2 border-[#FAF8F5] dark:border-[#1A1A1A]"
                style={{ backgroundColor: item.color }}
              />
              <span className="text-xs text-[#8A8680] font-[family-name:var(--font-mono)] block mb-1">
                {item.period}
              </span>
              <h3 className="text-base font-[family-name:var(--font-display)] font-semibold">
                {item.role}
              </h3>
              <p className="text-sm text-[#F4A259] font-medium mb-2">
                {item.company}
              </p>
