...
export default function Original({ caseStudy }: Props) {
  ...
  return (
    <div className="pt-28 pb-12 md:pb-20" style={{ "--accent": accentColor } as React.CSSProperties}>
      <div className="max-w-3xl mx-auto px-6">
        <Link href="/work" className="text-sm text-[#707072] tracking-hover inline-flex items-center gap-1 mb-8">
          &larr; Back
        </Link>
        ...
        <ScrollReveal delay={0.2}>
          <p className="text-[#707072] leading-relaxed mb-8">{caseStudy.description}</p>
        </ScrollReveal>

        <ScrollReveal delay={0.25}>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10 p-6 rounded-xl bg-[#f5f5f5]/50">
            <div>
              <span className="text-xs text-[#707072] block mb-0.5">Client</span>
              <span className="text-sm font-medium">{caseStudy.client}</span>
            </div>
            <div>
              <span className="text-xs text-[#707072] block mb-0.5">My Role</span>
              <span className="text-sm font-medium">{caseStudy.role}</span>
            </div>
            <div>
              <span className="text-xs text-[#707072] block mb-0.5">Industry</span>
          ...
          <ScrollReveal delay={0.3}>
            ...
            >
              ...
              <p className="text-2xl md:text-3xl font-[family-name:var(--font-display)] font-bold" style={{ color: accentColor }}>
                {isNumericMetric ? (
                  <><AnimatedCounter to={metricValue} />{caseStudy.metric.value.replace(/[0-9]/g, "")}</>
                ) : caseStudy.metric.value}
              </p>
              <p className="text-sm text-[#707072] mt-1">{caseStudy.metric.label}</p>
            </div>
          </ScrollReveal>
        )}
              ...
              <div className="space-y-6">
                {section.items.map((item, j) => (
                  <ScrollReveal key={j} delay={Math.min(j * 0.05, 0.2)}>
                    <OriginalSectionRenderer item={item} />
                  </ScrollReveal>
                ))}
              </div>
            </section>
...
function OriginalSectionRenderer({ item }: { item: CaseStudySection }) {
  switch (item.type) {
    case "text": {
      ...
      return (
        <div>
          <ParallaxWrapper className="w-full rounded-lg">
            <Image src={item.image!.src} alt={item.image!.alt} width={1920} height={1080} sizes="(max-width: 768px) 100vw, 768px" className="w-full h-auto rounded-lg" />
          </ParallaxWrapper>
          {item.image?.caption && <p className="text-xs text-[#707072] mt-2 leading-relaxed">{item.image.caption}</p>}
        </div>
      );
    case "image-grid":
      return (
        <div className="space-y-4">
          {item.images?.map((img, k) => (
            <div key={k}>
              <ParallaxWrapper className="w-full rounded-lg">
                <Image src={img.src} alt={img.alt} width={1920} height={1080} sizes="(max-width: 768px) 100vw, 768px" className="w-full h-auto rounded-lg" />
              </ParallaxWrapper>
              {img.caption && <p className="text-xs text-[#707072] mt-2 leading-relaxed">{img.caption}</p>}
            </div>
          ))}
        </div>
      );
    case "quote":
      return (
        <blockquote className="border-l-2 pl-5" style={{ borderColor: "var(--accent)" }}>
          <p className="text-[#111111] italic leading-relaxed">{item.quote}</p>
          {item.author && <footer className="text-sm text-[#707072] mt-2">&mdash; {item.author}</footer>}
        </blockquote>
      );
    case "metrics":
      return (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {item.metrics?.map((m, k) => (
            <div key={k} className="p-5 rounded-xl bg-[#f5f5f5]/50">
              <p className="text-xl md:text-2xl font-[family-name:var(--font-display)] font-bold">{m.value}</p>
    ...
    case "embed": {
      ...
      return (
        <iframe src={item.embed!.url} title={item.embed!.title} className={`w-full rounded-lg ${isVideo ? "aspect-video" : "h-[900px]"}`} allowFullScreen allow="autoplay; fullscreen" />
      );
    }
    case "before-after":
